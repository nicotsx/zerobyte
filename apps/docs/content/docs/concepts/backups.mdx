...
Backups inherit the compression mode from the repository they write to. Each backup job can optionally override it, so jobs writing to the same repository can use different compression modes. On the backup job form, choose **Inherit from repository** (default) to follow the repository setting, or pick **off**, **auto**, or **max** to override it for that job only.
