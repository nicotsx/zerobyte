...
| Configuration export passphrase | Decrypting one `.zbex` configuration export                              | One exported file          | Chosen or generated when exporting                              |
...
Organization owners and admins can download a `.zbex` file containing the organization's repositories, volumes, schedules, notification destinations, and recovery key. Zerobyte decrypts only those exported values in memory and encrypts the complete file with a dedicated export passphrase.

The export passphrase is separate from your account password, recovery key, and `APP_SECRET`. Zerobyte does not store it and cannot recover it.

To restore the configuration during onboarding on another instance, you need:

- The `.zbex` file
- Its export passphrase

The source instance's `APP_SECRET` is not exported or required. After import, the destination instance encrypts the imported credentials using its own `APP_SECRET`.

<Callout type="warn">
	Store the export and its passphrase separately. Anyone with both can decrypt the credentials and recovery key
...
- Keep each configuration export passphrase with the same care as the export itself; neither can be recovered from the other.
