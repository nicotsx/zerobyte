...
<Callout type="warn">
	Never share or commit your `APP_SECRET`. If you lose it, encrypted data (credentials stored for volumes and
	repositories) cannot be recovered.
</Callout>

<Callout type="info">
	Zerobyte reads the contents of `APP_SECRET_FILE`, trims surrounding whitespace and newlines, and applies the same
	32-256 character requirement as `APP_SECRET`.
</Callout>
...
| `GOMAXPROCS`              | Optional positive integer passed to Restic processes to limit CPU scheduler threads. See [Performance tuning](/docs/guides/performance-tuning).                   | Restic default         |
...
| `SERVER_IDLE_TIMEOUT`     | Server idle timeout in seconds. Set `0` to disable it; Bun supports up to `255` seconds.                                                                          | `60`                   |
...
| `ZEROBYTE_AGENT_ROOTS`    | Standalone agent JSON allow-list of filesystem roots. Each entry has `id`, `label`, `path`, and optional `allowBackup` (default `true`).                          | `[]`                   |
| `ZEROBYTE_CONTROLLER_URL` | Standalone agent outbound WebSocket URL, normally `wss://host/api/v1/agents/connect`. The public endpoint is enabled in production Bun builds, not `bun run dev`. | (none)                 |
| `ZEROBYTE_AGENT_TOKEN`    | Standalone agent machine credential. Enrollment exchanges its one-time code for this credential; do not set this to the enrollment code.                          | (none)                 |

### Trusted agent filesystem roots

Standalone agents expose local files through an explicit allow-list. Enrollment saves the credential and roots; use the [Remote agents](/docs/guides/remote-agents) guide for that flow. When configuring a saved machine credential through environment variables, set all three agent variables. For example:

```bash
ZEROBYTE_AGENT_ROOTS='[{"id":"data","label":"Application data","path":"/srv/data"}]'
```

IDs and labels must be unique and non-empty. Paths are expanded and canonicalized by the agent at startup; invalid or missing paths prevent startup. Only the ID, label, and backup capability are advertised to the controller. Absolute host paths are never sent. Remote restore is not supported. Enrolled CLI agents load their saved roots when this variable is absent. When both `ZEROBYTE_AGENT_TOKEN` and `ZEROBYTE_CONTROLLER_URL` are set, the agent uses environment configuration instead; set `ZEROBYTE_AGENT_ROOTS` as well or it advertises no filesystem roots. The supervised built-in local agent separately provides its compatibility root for existing local workflows.
...
<Callout type="warn">
	**Do not** point `/var/lib/zerobyte` to a network share. This causes permission issues and severe performance
	degradation. Always use local storage.
</Callout>
...
<Callout type="info">
	If `BASE_URL` starts with `https://`, browsers will only send auth cookies over HTTPS connections. Plain HTTP access
	may show the login page but authentication will fail.
</Callout>

<Callout type="warn">
	`TRUSTED_ORIGINS` only allows additional origins for CORS. It does **not** disable secure cookies or make HTTP access
	work when `BASE_URL` is HTTPS.
</Callout>
...
<Callout type="info">
	The standard volume and repository forms in the UI currently store the value you enter, encrypted at rest. `env://`
	and `file://` references are only resolved during provisioning.
</Callout>
...
<Callout type="warn">
	Always check the [release notes](https://github.com/nicotsx/zerobyte/releases) before updating, especially for v0.x.x
	versions which may include breaking changes.
</Callout>
