...
const migrateSnapshotsToShortIdTag = async (): Promise<MigrationResult> => {
	...
	for (const schedule of backupSchedules) {
		...
		const args: string[] = ["--repo", repoUrl, "tag", "--tag", oldTag, "--add", newTag];

		addCommonArgs(args, env);

		logger.info(`Retagging snapshots for schedule '${schedule.name}' from tag '${oldTag}' to '${newTag}'`);
		...
		logger.info(`Retagged snapshots for schedule '${schedule.name}' from tag '${oldTag}' to '${newTag}'`);
	}
