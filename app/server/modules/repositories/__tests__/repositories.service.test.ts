...
describe("repositoriesService.restoreSnapshot", () => {
	...
	test("rejects remote restore requests that do not match the target agent platform", async () => {
		const organizationId = session.organizationId;
		const agentId = `agent-${randomUUID()}`;
		const repository = await createTestRepository(organizationId, {
			type: "s3",
		...
		).rejects.toThrow("Original location restore is unavailable");
	...
	test("forwards selected Windows file restore requests to the restore executor", async () => {
		const { organizationId, userId, repositoryShortId, restoreMock } = await setupRestoreSnapshotScenario([
