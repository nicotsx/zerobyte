			...
			<div className="flex flex-col items-center gap-3 px-4 py-16 text-center">
				...
				<div>
					...
					<p className="mt-1 text-sm text-muted-foreground">
						{kind || outcome
							? "Choose different filters to broaden the task log."
				...
				<TableBody>
					{history.items.map((item) => (
						<TableRow key={item.id} className="h-14">
							<TableCell className="pl-4 font-medium">{item.taskType}</TableCell>
							<TableCell>
								...
								<Tooltip>
									<TooltipTrigger asChild>
										<Button
											variant="ghost"
											size="icon"
											aria-label={`View details for ${item.taskType} targeting ${item.target.label}`}
