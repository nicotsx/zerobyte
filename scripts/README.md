...
```bash
node scripts/create-test-files.ts [options]
```
...
node scripts/create-test-files.ts

# Create 50 files, 10K to 100K, with nested directories
node scripts/create-test-files.ts -c 50 --min-size 10K --max-size 100K -n

# Create 5 files, 1MB to 10MB
node scripts/create-test-files.ts -c 5 --min-size 1M --max-size 10M -o ./data/test-backup

# Create 100 small files in nested structure
node scripts/create-test-files.ts -c 100 --min-size 100B --max-size 1K -n
```
